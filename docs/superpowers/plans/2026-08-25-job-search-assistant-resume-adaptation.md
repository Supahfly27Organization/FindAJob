...
- Adaptation must never invent content the template doesn't contain, and must never drop a position/role from the template. Dropped content is checked via the self-reported `originalPositionCount`/`retainedPositionCount` count inside the same call (see "Key technical decision" above); invented content is addressed only by prompt instruction, with no independent verification of either. A detected shortfall is treated as a generation failure, not silently saved (Story 4.2).
